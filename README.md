...
- [ ] Jadwalkan **Backup & Restore** database secara rutin (mingguan/bulanan), terutama sebelum ganti perangkat. Daftar tabel yang di-backup sekarang otomatis mengikuti skema aplikasi (`lib/ensureDatabase.js`) — jadi kalau ke depannya ada modul/tabel baru lagi, backup otomatis ikut mencakupnya tanpa perlu diingat-ingat manual.
- [ ] Fitur **Potong Data/Tutup Buku** (di halaman yang sama) sekarang juga mencakup data-data baru: Setoran & Riwayat Kocok Arisan, Pengumuman, Surat-Menyurat, dan Koperasi Syariah. Khusus **Pinjaman Koperasi**, yang bisa dipotong **cuma yang statusnya sudah Lunas** — pinjaman yang masih berjalan tidak akan pernah ikut kepotong walau tanggal pinjamnya masuk rentang yang dipilih, supaya piutang yang belum lunas tidak hilang catatannya.
- [ ] Proses **Restore** sekarang juga memvalidasi isi file backup lebih ketat (nama kolom di dalamnya dicek dulu sebelum dipakai) — kalau ada file backup yang rusak/aneh, restore otomatis dibatalkan dengan pesan jelas, tidak diam-diam dijalankan.
